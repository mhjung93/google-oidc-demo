...
// 리프는 둘 다에서 나온다(V8, 2026-09-24): 사용자 리프 Poseidon(4, Cf_u) 와 세션 리프 Poseidon(5, Cf_s) 를 같은 트리·같은
// root 에 대해 각각 비멤버십으로 증명한다(④·④′). uid·s_u 는 C_u 안에 있어 서명이 C_u 를 덮는 것으로 PPID 의 출처가 묶인다.
//
