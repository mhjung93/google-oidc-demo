...
- **`pk_i` 영속성으로 인한 추적 폭발반경**: B1(Task 4)에서 `wallet_agent.js`가 `sk_i`/`pk_i`를
  지갑 프로세스마다 영속화하기로 결정했다 — 즉 로그인마다 새로 생성되는 게 아니라 같은
  지갑 프로세스에서는 계속 같은 `pk_i`가 쓰인다. 이걸 B2의 추적과 결합하면, `pk_i` 하나를
  한 번 추적(open)하는 것만으로 그 지갑이 과거·미래에 보낸 **모든** 온체인 PPID
  트랜잭션이 같은 `uid`로 연결된다 — 분쟁이 된 그 트랜잭션 하나만 열리는 게 아니다. B2
  코드 자체의 결함은 아니고(매칭 로직은 항상 `(pk_i, max_height, rp_nonce)` 전체 튜플로
  세션을 정확히 구분한다) B1+B2를 함께 놓고 봤을 때 드러나는 emergent 성질이다. 인가
  절차를 도입할 때(위 항목) 이 폭발반경("한 번의 승인이 전체 이력을 연다")을 승인
  범위/정책에 명시적으로 반영할 필요가 있다.
