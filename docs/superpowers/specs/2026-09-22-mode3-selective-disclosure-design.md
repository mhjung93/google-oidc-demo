...
- 열린 것: (a) 로그인 때 서비스가 공개를 요구하는 정책(지금은 기록만). (b) 속성 슬롯 의미·데모 값. (c) 관리자 속성 변경 시
  옛 리프 게시 여부 — 이중 구조의 "속성 변경 시각 상관" 결정(보류 중)과 같은 항목이며, 세 번째 선택지로 "지갑이 revoked 를
  만나면 세션을 버리지 않고 같은 세션키로 재발급 뒤 재검증"(서비스에는 보통 재검증으로 보임)을 추가한다.
