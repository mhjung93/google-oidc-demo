...
- 세션키 서명 `σ` 의 다이제스트는 지금처럼 `(chainid, wallet, to, value, data, nonce)` 를 덮는다 — 공개 값은 π 의 공개 입력으로
  ...
  두 개**(예: 어떤 대상엔 정확한 값 공개, 다른 대상엔 구간 공개 — mask 는 같고 lo/hi 만 다름)가 동시에 있으면 릴레이어가 하나를
  골라 붙일 수 있다 → 지갑은 트랜잭션마다 σ 다이제스트에 공개 값 9워드 전부(`disc_mask`·`disc_lo[4]`·`disc_hi[4]`)를 포함해
  `keccak256(abi.encode(chainid, wallet, to, value, data, nonce, pub[14], pub[15..18], pub[19..22]))` 로 서명하고 컨트랙트도 같은
...
π_u V2 는 prove/verify 모두 V5 보다 빠르다(121.9/140.5 → 79.7/98.7 ms). §3.4 대로 증명 쪽 스칼라곱이 9 → 5 로 줄어든 것은 prove가
빨라진 설명이 되지만, 검증 쪽 스칼라곱은 9 → 11 로 오히려 **늘었는데도** 실측 verify 는 줄었다(140.5 → 98.7 ms) — **이 감소의
원인은 측정하지 않았다**(후보: z_attr 4개의 스칼라곱이 사라지고 AA 가 아는 항(uid·attrs)에 대한 계산이 스칼라 곱셈이 아니라
고정 기저 곱으로 처리됐을 가능성, 측정 잡음). "스칼라곱이 줄어 검증도 빨라졌다"는 식의 뭉뚱그린 설명은 근거가 없다.
