...
**개정 2026-09-15 — `2026-09-15-mode3-session-statement-design.md` 가 이 문서의 §3(서명 메시지의 `nonce`)·§4·§5·§6·§7 을
대체한다.** `nonce` 는 서비스가 뽑는 `r_s` 로 바뀌어 공개 입력이 되고, 성명은 세션 단위가 되며, 서비스 인증서 `cert_s` 가
들어온다. 커밋 구성·가명 유도·속성 슬롯은 그대로다. 2026-09-14 설계 대화에서 정해진 내용을 옮긴다.
