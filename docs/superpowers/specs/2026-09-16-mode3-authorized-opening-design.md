...
- **CIA 와 서비스가 담합하면 운영자 승인 없이 그 서비스의 태그를 전부 연다.** 두 조각을 합치면 되기 때문이다. 기록 결합 판에서도
  ...
  필요하며, 태그 방식은 그 확장이 자연스럽다(§8 3번, 이 문서 범위 밖).
...
- **CIA 조각 `x_AA,s` 는 승인 시점에 만든다.** 승인 전 서비스는 조합 키가 없으므로 지갑이 태그를 만들 수 없고, 따라서 로그인이 없다.
  조각은 `cia_state.json` 에 있다 — 잃으면 그 서비스의 과거 태그는 영원히 열 수 없다(§8 5번). `sk_CIA` 와 같은 급으로 다룬다.
...
- **`arid` 는 요청 시점에 배정한다.** 거절된 origin 은 거절 상태로 남고 재요청은 403 이다(§8 6번).
- **데모 각본에 "0. 서비스 등록 승인" 행이 생긴다.** 격리 테스트·데모 스택 테스트는 헬퍼가 관리자 시크릿으로 승인을 대행한다.
- v3 상태 파일의 기존 `rps` 항목(키 없음)은 마이그레이션 때 `status: "approved"` 로 두되 조각이 없다. 그 서비스가 키를 내며
  재등록하면 `status` 를 다시 `pending` 으로 돌려 운영자 승인을 받게 한다 — 그래야 무허가 키 바인딩으로 진짜 서비스가 잠기지
  않는다. 승인해야 조각을 만들고 `pk_trace` 를 채운다(§7).
...
① 요청   RP:        로그에서 그 PPID 의 트랜스크립트 하나를 고른다. c1 = (publicSignals[11], [12])
                    ...
                    sig = EIP-191.Sign(sk_service, "mode3-open:" + arid + ":" + r_s + ":" + PPID + ":" + D_svc.x + ":" + D_svc.y + ":" + ts)
...
- **CIA (v4)**: `accounts`, `issued`, `rps`(arid → {name, origin, pk_service, X_svc, **x_AA,s**, pk_trace, status, requestedAt, decidedAt}),
  **`openings`**(영구), `revoked`, `pending`, `epoch`. `used_rs` 삭제. 새 env: `MODE3_VKEY_PATH`. **v3 → v4 마이그레이션**: `used_rs`
  버림, `rps` 항목은 `status: "approved"`·조각 없음, `openings = []`. 그런 항목이 키를 내며 재등록하면 `status` 를 `pending` 으로
  되돌려 운영자 승인을 다시 받는다(§3). v2 이하는 기동 거부(그대로).
...
8. **트랜스크립트 보관 비용.** 로그인당 약 1.7 KB(공개 입력 14개)가 서비스에 쌓인다. 서비스가 잘라내면 그 세션은 열 수 없다.
   PPID 당 하나만 남기면 충분하다.
9. **회로 비용.** 약 +25% 제약, zkey 재생성.
10. **태그는 사용자 프라이버시를 조각 둘의 비밀성에 건다.** 두 조각이 모두 새면 그 서비스의 모든 과거 로그인이 열린다. 전방 비밀성은
    없다.
11. 세션 설계 §9 의 나머지(CIA 가 로그인 경로에 있음, 속성 진위, 세션 요청 신선도)는 그대로다. "등록 무허가"는 사라진다.
