...
**상태: 설계 확정. 구현 전.**
**개정 2026-09-16 — `2026-09-16-mode3-authorized-opening-design.md` 가 이 문서의 §2(조건부 추적 가능성: 범위 밖 → 2-of-2 트레이스 태그로 승인된 개봉)·§3(서비스 등록 승인, 두 서비스 키, `cert_s` 메시지 V2 에 `pk_trace`)·§4(인증 요구에 `pk_trace`)·§6(공개 입력 9 → 14, 조건 ⑤ 태그)·§8(CIA 의 `used_rs` 삭제)·§9 1번을 대체한다.**
