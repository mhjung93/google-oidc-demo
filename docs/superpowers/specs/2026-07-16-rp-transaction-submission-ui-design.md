...
- B2 추적 호출을 위한 별도 UI
- **가스비 지불 프라이버시(릴레이어 서비스) — 구체적 메커니즘 메모(2026-07-17)**: 지금은
  MetaMask에 연결된 계정이 `execute()`의 가스비를 직접 내고, `PPIDWallet`이 그 계정에
  아무것도 상환해주지 않는다. `PPIDWallet_ppid`에 미리 자금을 넣어둬도, 바깥쪽(outer)
  트랜잭션 자체는 이더리움 프로토콜 제약상 항상 실제 EOA가 서명·가스 선지불해야
  하므로 이 자체는 못 피한다. 다만 `execute()`에 "실행 성공 후 호출자에게 소모한
  가스만큼 컨트랙트 잔액에서 환급"하는 로직을 추가하면(ERC-4337의 paymaster/relayer
  패턴과 동일), 제3자 릴레이어가 대신 가스를 내고 컨트랙트 잔액에서 상환받는 구조가
  가능해져서 `sk_i`/`pk_i` 소유자가 자기 자금을 전혀 안 가져도 되게 만들 수 있다.
  `PPIDWallet.execute()`(`contracts/PPIDWallet.sol`) 자체를 수정해야 하는 작업이라
  B1 범위를 다시 여는 일이고, 이번 스펙/계획에서는 다루지 않는다.
