...
# v5 (2026-09-23): make_zklogin_slides.py 복사본 — 자격증명 이중 구조(C_u 1회·C_s 세션) 두 칸 반영, 출력 260923.
...
DST = 'documents/260923_zkDelegation_vs_zkLogin.pptx'
...
table_slide('비교 1 — 발급자·가시성·회로·주소', H, [
    ['발급자', '무수정 OIDC 제공자. JWT를 그대로 씀', '협력하는 AA. 사용자 자격증명 C_u 에 1회(π_u), 세션 커밋 C_s 에 세션마다 EdDSA 서명(증명 없음)', '제공자가 협력해야만 "무엇에 서명하는지 모르게" 만들 수 있다. G4·G5·폐기·개봉이 전부 이 전제 위'],
    ['제공자가 보는 것', '클라이언트(aud)·사용자 sub', 'uid·Cf_u·C_s·chainid·max_height·allowAgent. 서비스·가명·세션키·속성값은 못 봄', 'OIDC의 근본 누수(어느 서비스에 로그인하는가)를 없애는 것이 목표'],
    ['회로 안 서명 검증', '제공자의 RSA(JWT). 제약 수백만, 외부 증명 서비스가 보통 필요', 'Baby Jubjub EdDSA-Poseidon. 제약 25,369(V6), 지갑에서 0.81 s', '지갑이 스스로 증명하려면 회로 친화 서명이어야 한다. 협력 발급자라 가능'],
...
table_slide('비교 2 — 비밀·유효기간·폐기·세션', H, [
    ['salt / 사용자 비밀', 'salt 서비스가 보관·백업', 's_u는 지갑에, AA에는 커밋 cm_u만. 자격증명 1회에 같은 값임을 증명(π_u), 세션은 서명만', '비밀을 제3자에 맡기지 않으면서 불변성(G2) 강제. 대가: s_u 분실 = 주소 분실(복구 미해결)'],
