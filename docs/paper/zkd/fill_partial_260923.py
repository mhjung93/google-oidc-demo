# documents/260921_partial.pptx(사용자 뼈대)를 채운다 → documents/260923_partial_filled.pptx. 원본은 건드리지 않는다.
# 2026-09-23 판: V6 선택 공개(공개 입력 23, π_u V2 속성 공개)·MetaMask/Snap·증분 트리 동기화·전체 점검·논문 v5 반영. 260921 판 스크립트는 그대로 둔다.
...
DST = 'documents/260923_partial_filled.pptx'
...
table(g, X0, Y0 - 250000, W0 // 2 - 60000, 4900000,
      ...
      [['uid', '사용자 ID. AA 계정 식별자(이메일 같은 것). AA 는 안다'],
       ...
       ['a₁..a₄', '사용자 속성 4슬롯(생년, 국가, 등급, 예비; 64비트). AA 계정 기록이 출처(관리자만 변경), 지갑은 사본. 구간(lo ≤ a ≤ hi)만 골라 공개 가능'],
...
label(m, X0, Y0 + 3350000, W0, 900000,
      '무엇이 같은가: 서비스가 보는 것(π_rp 공개 입력 — 2026-09-22 선택 공개로 23개), 가명 PPID, 온체인 검사, 조건부 추적. 보안 불변식(스펙 §2 a–g)은 그대로 성립한다.\n'
...
table(s, X0, Y0 + 430000, W0, 2600000,
      ...
      [['언제', '사용자 자격증명 발급 — 첫 로그인, 속성 변경 때만', '로그인·재검증마다(캐시)', '트랜잭션마다 첨부(캐시 π 재사용)'],
       ...
       ['공개 입력', 'uid(사용자 ID), a₁..a₄(AA 기록), C_u(사용자 커밋), cm_u(등록 커밋)', 'PPID(가명), arid(서비스 ID), pk_i(세션 공개키), max_height(만료), chainid, allowAgent, root(폐기 트리 루트), pk_AA(AA 키), pk_trace(추적 키), tag(추적 태그 c1,c2), disc_mask·disc_lo[4]·disc_hi[4](공개 술어) — 23개', '같음 + 트랜잭션 내용(to, value, data, nonce) + 공개 9워드'],
       ['증명하는 것', '사용자 커밋 C_u 가 인증된 uid, 등록 때의 비밀값 s_u, AA 기록의 속성 a₁..a₄ 로 만들어졌다', 'AA 서명이 (C_u, C_s, 만료, 체인, 플래그)를 덮고, 가명 PPID 가 그 안의 (uid, s_u) 에서 나왔고, C_u 의 리프가 폐기 트리에 없고, 추적 태그가 같은 uid 로 만들어졌고, mask 가 켜진 슬롯은 lo ≤ aₖ ≤ hi 다', '같음. 증명 밖에서 세션키 서명 σ = Sign_sk_i(keccak(체인, 계정, to, value, data, nonce))'],
...
label(s, X0, 860000, W0, 300000, '시그마 프로토콜(Fiat–Shamir, Baby Jubjub). 회로·zkey·신뢰 셋업 없음. 증명 80 ms / 검증 99 ms(2026-09-22 실측, V2). 사용자당 한 번 — 속성이 바뀔 때만 다시 만든다. 속성은 AA 가 자기 기록에서 넣어 검증한다(V2, 2026-09-22).', size=12, color=GREY)
# 왼쪽: 공개 / 증인
...
box(s, X0, 1580000, LW1, 1150000, 'uid — 사용자 ID(인증에 쓴 계정)\na₁..a₄ — AA 계정 기록의 속성(요청에서 받지 않음)\nC_u — 지갑이 낸 사용자 자격증명 커밋(곡선 점)\ncm_u — 등록 때 낸 비밀값 s_u 의 커밋', size=11, align=PP_ALIGN.LEFT)
...
box(s, GX1, 1580000, LW2, 1600000,
    'C_u − uid·G_UID − Σaₖ·G_Aₖ = s_u·G_SU + blind_u·H      (uid·aₖ 는 AA 가 자기 값을 빼서 검사)\n∧  cm_u = s_u·G_SU + r_u·H          (G_*, H 는 고정 생성원)\n두 식의 s_u 가 같다 — "등록 때 고정한 비밀값과 같은 값으로 커밋했다"가 핵심(사용자당 주소 하나, G2). 다른 속성으로 만든 C_u 는 결합성 때문에 통과 못 한다',
    size=12, align=PP_ALIGN.LEFT)
box(s, GX1, 3280000, LW2, 1180000,
    '프로토콜: 지갑이 T1, T2(무작위 커밋) → c = Poseidon(D_USERCRED_V2, uid, a₁..a₄, C_u, cm_u, T1, T2) → 응답 z_su, z_blind, z_ru\n'
    'AA 검증: 점 부분군 검사 → z < ℓ → c 재계산 → 두 등식\n  z_su·G_SU + z_blind·H = T1 + c·(C_u − uid·G_UID − Σaₖ·G_Aₖ),   z_su·G_SU + z_ru·H = T2 + c·cm_u',
...
label(s, X0, 4600000, W0, 700000,
      'AA 가 알게 되는 것: "C_u 는 이 사용자 ID, 등록된 비밀값, 내 기록의 속성으로 만들어졌다" 뿐. 서비스 ID·세션키는 이 커밋에 없고, 비밀값·블라인드는 커밋의 hiding 으로 숨는다. 속성값은 AA 가 원래 아는 것이고 AA 가 모르는 것은 "어디서 공개되는가"(G5).\n'
...
label(s, X0, 860000, W0, 300000, 'Groth16(BN254), 25,369 제약(V6 2026-09-22; V5 26,601, V4 25,560). 증명 809 ms, 검증 10.7 ms. 공개 입력 23개(14 + 공개 술어 9)의 순서는 서비스·컨트랙트가 의존한다.', size=12, color=GREY)
# 회로 상자
...
# 공개 입력 (왼쪽)
pubs = ['PPID (가명)', 'arid (서비스 ID)', 'pk_i (세션 공개키)', 'max_height (만료)', 'chainid (체인)', 'allowAgent (플래그)', 'root (폐기 트리)', 'pk_AA (AA 키)', 'pk_trace (추적 키)', 'tag c1, c2 (태그)', 'disc_mask, lo[4], hi[4] (공개 술어)']
ph_ = CH // len(pubs)
label(s, X0, CY - 330000, 2000000, 300000, '공개 입력 (검증자가 본다, 23개)', size=11, bold=True, color=BLUE)
...
blocks = [
    ...
    (CX + 5250000, CY + 150000,  '범위·술어 검사\npk_i < 2¹⁶⁰, max_height < 2⁶⁴, aₖ < 2⁶⁴,\nallowAgent ∈ {0,1}, r ≠ 0; mask 켜진 k: lo_k ≤ a_k ≤ hi_k', LIGHT),
...
label(s, CX + 150000, CY + 2500000, CW - 300000, 700000,
      '조건 ①~⑦: AA 서명(①) · 커밋 재계산으로 공개 세션키·서비스 ID 바인딩(②) · 가명 유도(③) · 폐기 비멤버십(④) · 추적 태그(⑤) · 범위(⑥) · 선택 공개 술어(⑦, 로그인은 mask 0).\n'
      '증명 밖: σ = Sign_sk_i(r_s) — 서비스가 준 챌린지에 대한 세션키 서명. 지갑은 (root, r_s) 키로 증명을 캐시하고 root 가 바뀔 때만 다시 만든다.',
      size=10, color=GREY)
label(s, X0, CY + CH + 100000, W0, 600000,
      '서비스 검사 순서: 챌린지 r_s 소비 → 체인에서 root·마지막 게시 블록·현재 블록 읽기 → root 일치 → 마지막 게시 ≤ 100 블록(2026-09-23, 온체인과 같은 상한) → 현재 블록 ≤ max_height ≤ 현재 블록 + L → chainid·pk_AA·arid·pk_trace 가 설정값 → allowAgent ≤ 1, c1 ≠ O, mask < 16 → Groth16 → σ.  통과하면 세션 테이블에 {PPID, pk_i, max_height, allowAgent, root, 공개 술어(비마스크 슬롯은 0)}.',
      size=10, color=GREY)
...
label(s, X0, 860000, W0, 300000, '새 증명이 아니다. 로그인 때 만든 π_rp(공개 입력 23개 포함)를 execute() 에 그대로 첨부하고, 권한은 세션키 서명 σ 가 payload·nonce·공개 9워드를 덮는 것으로 준다. 속성 구간을 공개하는 트랜잭션만 새 π.', size=12, color=GREY)
# 왼쪽: 지갑
box(s, X0, 1200000, 3300000, 330000, '지갑 (트랜잭션마다)', size=12, bold=True, fill=PALE)
box(s, X0, 1530000, 3300000, 3050000,
    '1. 체인 동기화(체크포인트+델타) → 현재 root. 캐시에 (root, r_s, 공개) 의 증명이 있으면 재사용, 없으면 π_rp 재증명(≈ 0.8 s)\n'
    '2. 계정 주소 = factory.computeAddress(PPID)  (CREATE2). 코드가 없으면 릴레이어가 deploy(PPID)\n'
    '3. nonce = 계정 컨트랙트의 현재 nonce\n'
    '4. digest = keccak(chainid, 계정 주소, to, value, data, nonce, disc_mask, disc_lo[4], disc_hi[4]);  σ = Sign_sk_i(digest)  (세션키 서명)\n'
    '5. execute(payload, σ, 증명(a, b, c), 공개 입력 23개) 를 릴레이어(또는 브라우저 데모의 MetaMask)로 제출',
    size=11, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP)
...
box(s, X0 + 3700000, 1530000, 4300000, 3050000,
    ...
    '⑨ nonce += 1 → 실제 호출 to.call{value}(data ‖ 공개 9워드 — 항상 붙임, 빈 data·mask 0 만 예외) → 이벤트 Executed, Mode3Auth(pk_i, max_height, allowAgent, tag), Disclosure',
    size=11, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP)
# 오른쪽: 태그·비용
...
box(s, X0 + 8200000, 3350000, 3320000, 1230000,
    '비용 (2026-09-22 실측, V6)\nexecute 402,130 gas(캐시 π) / 419,262(첫 실행) / 444,509(두 슬롯 공개 + AttrGate)\n캐시 π 왕복 151 ms(새 π 1.0 s), 계정 배포 1회 852k gas, 게시(리프 1) 43.9k, 하트비트 40.3k\n같은 π 재사용 = 같은 태그 암호문 — sender 가 PPID 계정이라 어차피 같은 주소로 이어져 있어 새로 드러나는 것은 없다',
...
# 2 체인 동기화 (읽기만)
node(0, 2, '트리 동기화(체크포인트+델타)'); node(3, 2, 'RevocationLog')
msg(3, 0, 2, '마지막 블록 이후 Revoked · root (읽기)', dashed=True, color=GREEN)
# 3 사용자 자격증명 (사용자당 한 번)
...
table(s, X0, Y0 - 100000, W0 // 2 - 100000, 2800000,
      ...
      [['프로세스 4', 'AA(cia.js), 지갑 에이전트(mode3_wallet_agent.js), 서비스(mode3_rp.js), hardhat 노드'],
       ['컨트랙트 5', 'RevocationLog, PiCredVerifier(snarkjs 생성), Mode3WalletFactory(서비스별, isWallet), Mode3Wallet(사용자·서비스별, CREATE2), AttrGate(공개 속성 소비 데모)'],
       ...
       ['테스트', 'unit·circuit·contract·chain·browser(Playwright, 실제 Snap 코드)·snap 그룹(격리 인스턴스), 데모 스택 시나리오 32']],
      [1.1, 4.0], size=10)
table(s, X0 + W0 // 2 + 100000, Y0 - 100000, W0 // 2 - 100000, 2800000,
      ['측정 (2026-09-22·23, V6, Ryzen 5950X, N=10 중앙값)', '값 (괄호는 V5 2026-09-21)'],
      [['π_rp 제약 / 증명 / 검증', '25,369 / 809 ms / 10.7 ms  (26,601 / 860 / 11.5)'],
       ['π_u(시그마) 증명 / 검증', '80 ms / 99 ms — 사용자당 1회  (122 / 141)'],
       ['로그인 왕복(세션 발급+증명)', '1,054 ms (세션 발급 124, 증명 ≈ 810)  (1,073)'],
       ['재검증(캐시 π) / 트랜잭션 왕복(캐시 π / 새 π+공개)', '31 ms / 151 ms / 1,000 ms'],
       ['execute gas(캐시 π / 첫 실행 / 두 슬롯 공개) / 계정 배포', '402,130 / 419,262 / 444,509 / 852k  (339k / 356k)'],
       ['지갑 트리 동기화, 1,000 리프(전체 재생 / 델타 1 / 델타 0)', '141 / 40 / 23 ms — 체크포인트+델타(2026-09-23)'],
       ['게시(리프 1) / 하트비트 gas', '43,888 / 40,305']],
      [2.2, 2.6], size=10)
...
label(c, X0, Y0 - 250000, W0, 330000, '출처(2026-09-23 원문 대조): zkLogin = Sui, CCS\'24(arXiv 2401.11735) §4–5 · zkAA = zero-knowledge address abstraction, SAC\'25(eprint 2023/191) §V–VI + zkaa-circom 코드 · BAAR = 익명 속성 자격증명, PLOS One 2026 e0343696.', size=11, color=GREY)
table(c, X0, Y0 + 120000, W0, 4300000,
      ['항목', 'zkLogin', 'zkAA', 'BAAR', 'zk-Delegation (이 연구)'],
      [['발급자(IdP) 개입', '로그인마다 OIDC JWT. IdP 가 aud(앱)·nonce(세션키 커밋) 를 본다', '기관이 cert(JWT) 1회 발급 → 온체인 등록 1회. 이후 Publish 마다 증명만, 발급자 없음', '자격증명 발급 1회(CIA), 제시마다 발급자 없음', '사용자 자격증명 1회(π_u) + 세션마다 서명(ZKP 없음). AA 는 서비스·세션키를 못 본다'],
       ['사용자 비밀(salt)', 'salt 서비스(앱/제3자, enclave·MPC) 또는 사용자 보관', 'cert(기관 서명) 자체가 비밀. 유출·분실 = 신원 전체', '사용자 비밀키', 's_u 는 지갑만. 등록 커밋 cm_u 로 AA 에 결합(주소 유일성)'],
       ['폐기', '없음(만료 epoch 만, T_max < T_cur + δ)', '없음(계약이 만료 조건을 "둘 수 있다"고만 언급). 게시 회로에 상태 조회 없음', 'accumulator 갱신, 선택적 폐기', 'RCL 리프(사용자당 1) + 온체인 root, N=1 신선도·하트비트, AA 없이 검증'],
       ['조건부 추적(개봉)', '없음(salt 서비스 평문 운용 시 JWT·주소 연결 가능)', '없음', '없음("accountability" 는 폐기로만)', '2-of-2 태그(서비스+AA, 운영자 승인), 검증 가능 암호화, AA 조각 PoK'],
       ['서비스 간 연결성', '앱(aud)별 주소 → 비연결', 'id = H(cert) 하나를 모든 체인·dApp 에 재사용 → 전부 연결', '제시마다 비연결', '서비스·체인별 PPID 비연결, 서비스 안은 주소로 연속'],
       ['체인 무관성', 'Sui 전용(네이티브 검증)', 'EVM 컨트랙트(Ethereum·Polygon 실측). 주장: 계약 가능한 체인 어디나', '체인 밖(오프체인 제시)', '체인 무관: Groth16 검증자 컨트랙트 + chainid 가 PPID 에. 오프체인 로그인도 같은 증명'],
       ['트랜잭션당 비용', '네이티브 검증 2.04 ms(Ed25519 의 ~36배), 서명 1.3 KB. 증명 세션당 1회 2.78 s', 'Publish 마다 Groth16 검증 240k gas(등록 387k). 증명 2.7 s(논문) / circom 판 0.47 s', '해당 없음', 'execute 402k gas(공개 0개; 공개 4개 444k), 캐시 π 재사용. 증명 로그인당 1회 0.81 s']],
      [1.3, 2.2, 2.0, 2.0, 3.4], size=9)

# ---------- 11. Todo ----------
s = S[10]
set_bullets(body(s), [
    (0, '구현 완료(전부 push, feat/mode3-cia 48fdb5c): 자격증명 이중 구조(V5) → 속성 선택 공개(V6, 공개 입력 23) → MetaMask/Snap 구색(등록 비밀은 Snap, 동의 창 3종, RP 팝업 로그인) → 증분 트리 동기화(체크포인트+델타)'),
    (1, '전체 코드 점검(4 점검자, Critical 0·Important 8·Minor 33) 뒤 Important 8 + 주요 Minor 수정 — 오프체인 root 나이 fail-closed, 개봉 dup 키, 재승인 allowAgent 바인딩 등. 보고서 docs/audits/2026-09-23-mode3/'),
    (0, '논문 v5(2026-09-23) 반영 완료: 사용자 자격증명 절(V-B′), 세션 발급 무증명, Table 2·3, VI 자격증명 단위 폐기·오프체인 root 나이·트리 체크포인트, IX. 형식 문서 2편도 §9·주석으로 갱신'),
    (1, '남은 논문 손질: Fig.1 은 v5 로 재작도됨. 한계 절의 M4(하트비트 경고가 pk_CIA 를 env 로 박은 구성에서는 꺼짐) 한 줄'),
    (0, '기존 연구 비교표(앞 장) "(확인)" 칸 검토, zkLogin 비교 덱(260918_zkDelegation_vs_zkLogin.pptx) 과 통일'),
    (0, '후속(다음 장): 속성 술어 확장(시각 상대·집합), 검증자 주소를 cert_s 에(보안 공백, 아직 미구현), 폐기 게시 배치(한계로만), 셋업 ceremony(스코프 밖)'),
], size=15)
...
items = [
    ...
    ('3. 증분 트리 동기화', '완료(2026-09-23)', LIGHT,
     '했던 것: 메모리 트리 + 캐시 파일(공개 리프만) + 마지막 블록 이후 Revoked 이벤트만 적용, root 를 컨트랙트와 대조(다르면 창세기부터 재생 1회, 그래도 다르면 거절).\n'
     '실측: 1,000 리프에서 전체 재생 141 ms → 델타 1개 40 ms / 델타 0 23 ms. 캐시는 정확성에 관여하지 않는다(조작 8종 전부 root 대조에 걸림).\n'
     '남은 것: 없음(재조직이 있는 공개 체인에서는 되감김 검사 + root 대조가 안전망).'),
    ('4. 지갑 UI 확인 대화상자', '완료(2026-09-22, MetaMask Snap)', PALE,
     '했던 것: snap-mode3 가 등록 비밀(s_u·r_u·sk_u·blind_u)을 보관하고 로그인·속성 공개·자기 폐기 동의 창을 띄움. 트랜잭션은 MetaMask 확인 창(EOA 가 가스 — 한계). RP 는 지갑 페이지 팝업으로 로그인(postMessage 오리진·인증서 검사).\n'
     '남은 것: 재승인 창의 allowAgent 를 세션 값에 묶는 것까지 점검에서 고침. 실제 Flask 확장 동작은 수동 체크리스트(docs/MODE3_DEMO.md).'),
...
label(f, X0 + 2 * (FW + 120000), Y0 - 200000 + FH + 120000 + 300000, FW, FH - 300000,
      '남은 순서 제안: 2(보안, 미구현) → 1 확장. 3·4 는 완료. 5 는 논문 한계 절.\n셋업 ceremony(단일 기여)도 한계 절에만 둔다(스코프 밖).',
      size=11, color=GREY)
