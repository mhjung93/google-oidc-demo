# Figure 1 (v3, 2026-09-18): one login, one on-chain transaction, one authorized opening in zk-Delegation. PIL 7.
from PIL import Image, ImageDraw, ImageFont

S = 1.0
SF = 1.3
W, H = int(2600*S), int(2750*S)
...
names = {'aa': 'Authentication Authority (AA)', 'user': 'User / Wallet', 'svc': 'Service', 'chain': 'Chain (log + PPID account)'}
TOP = int(60*S)
...
lifelines(int(2650*S))

# registration (once)
y = int(215*S)
arrow('user', 'aa', y, '', 'registration (once):  cm_u = s_u·G3 + r_u·H', 'solid', GREY)
arrow('svc', 'aa', y + int(70*S), '', '', 'solid', GREY)
t = 'service registration (once):  name, origin, pk_service, X_svc  →  operator approves  →  arid, pk_trace, cert_s;  service deploys its account factory'
...
y = selfbox('user', y + int(30*S), ['verify cert_s under pk_AA — it covers (arid, origin, pk_trace);  origin == cert_s.origin',
                             'fresh (pk_i, sk_i), blind;  C_pt = Commit(uid, arid, s_u, pk_i, a1..a4; blind);  choose allowAgent'])
y += int(60*S)
arrow('user', 'aa', y, '(2)', 'uid, C_pt, chainid, allowAgent, π_issue, sig_u', 'solid')
y = selfbox('aa', y + int(30*S), ['not disabled;  chainid allowed;  sig_u;  π_issue (C_pt carries uid and the s_u of cm_u)',
                           'max_height = grid(head + TTL);  σ_AA = Sign(D_cred, C, max_height, chainid, allowAgent)   — nothing stored per login'])
y += int(60*S)
...
y = selfbox('user', y + int(30*S), ['read root and Revoked events from the chain;  PPID = Poseidon(uid, s_u, chainid, arid)',
                             'tag: r fresh, c1 = r·B8, c2 = Poseidon(uid, arid) + Poseidon(r·pk_trace);  π for Table 2 (1)–(5);  σ = Sign(sk_i, r_s)'])
y += int(75*S)
arrow('user', 'chain', y - int(20*S), '', 'root, Revoked events (non-membership witness)', 'dotted', GREY, both=True)
y += int(40*S)
arrow('user', 'svc', y, '(4)', 'PPID, arid, pk_i, max_height, chainid, allowAgent, root, pk_trace, tag (c1, c2), π, σ', 'dashed', BLUE)
y = selfbox('svc', y + int(30*S), ['consume r_s;  root == chain root, head ≤ max_height (5);  chainid, pk_AA, arid, pk_trace == mine;  c1 ≠ O',
                            'verify π, σ;  sessions[r_s] = {PPID, pk_i, max_height, allowAgent, root};  append transcript to private login log'])
y += int(75*S)
...
y += int(110*S)
arrow('user', 'chain', y, "(6')", 'execute(payload, Sign(sk_i, chainid ‖ account ‖ payload ‖ nonce), π, public inputs)   via a relayer', 'solid', GREEN)
y = selfbox('chain', y + int(30*S), ['account = CREATE2(factory, PPID);  nonce;  ECDSA under pk_i;  PPID, arid, chainid, pk_AA, pk_trace == mine;  c1 ≠ O',
                              'root == log.root, root age ≤ MAX_ROOT_AGE, block ≤ max_height;  verify π;  nonce++;  call;  emit tag, allowAgent'], color=GREEN)
...
ly = y + int(60*S)
items = [('solid', BLACK, 'seen by the AA (C, chainid, max_height, allowAgent)'),
         ('dashed', BLUE, 'never seen by the AA at login (arid, cert_s, pk_trace, PPID, pk_i, tag, attributes, r_s)'),
...
for i, (style, color, label) in enumerate(items):
    yy = ly + (0 if i < 2 else int(60*S)) + (int(60*S) if i >= 4 else 0)
    if i == 2 or i == 4: x = 100
...
img.save('docs/paper/zkd/fig1_login_v3.png', dpi=(300, 300))
