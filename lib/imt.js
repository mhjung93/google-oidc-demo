...
export async function createIMT(depth) {
  ...
  return {
    async insert(value) {
      ...
      // 용량을 넘으면 getRoot()가 조용히 부분 트리의 root를 돌려준다 — 그 root로 만든
      // 증명은 온체인 root와 어긋나고, 원인을 추적하기 어렵다. lib/imt_v2.js와 같은
      // 불변식을 여기에도 건다(이 v1 트리는 이제 테스트·벤치에서만 쓰이지만, 조용히
      // 틀린 값을 내는 것은 그 용도에서도 나쁘다).
      if (values.length >= 2 ** depth) {
        throw new Error(`revocation tree is full: ${values.length} leaves at depth ${depth}`);
      }
      values.push(v);
      values.sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
    },

    // values는 정렬된 평범한 배열이고 build()가 호출마다 트리 전체를 재구성하므로,
    // 보수할 연결 포인터가 없다 — 배열에서 빼면 끝이다.
    async remove(value) {
      const v = BigInt(value);
      if (v === 0n) throw new Error('0 is the anchor leaf and cannot be removed');
      const i = values.indexOf(v);
      if (i === -1) return false; // 없으면 no-op
      values.splice(i, 1);
      return true;
    },
