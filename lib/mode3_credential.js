...
// 2026-09-14: 속성 4슬롯(attr0..3)과 (exptime, chainid, nonce) 서명 메시지 — 설계 2026-09-14-mode3-attribute-credential-design.md §3.
...
export async function credMessage(C, exptime, chainid, nonce) {
  ...
  // 회로의 Num2Bits(250)(pi_cred.circom nonceRange) 과 정합 — 그 상한을 넘는 nonce 는 애초에 열 수 없다.
  if (nonce >= SCALAR_MAX) throw new Error(`credMessage: nonce 는 [0, 2^250) 범위여야 한다: ${nonce}`);
  const poseidon = await getPoseidon();
