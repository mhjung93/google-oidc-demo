...
//   CIA 조각의 지식 증명(2026-09-21): 서비스가 X_svc 를 먼저 내고 CIA 가 나중에 조각을 고르므로, CIA 가
//   X_AA = X′ − X_svc 로 고르면 pk_trace = X′ 의 이산로그를 CIA 혼자 알게 된다(rogue key). 승인 응답에
//   X_AA 와 Schnorr PoK 를 같이 실어 서비스가 pk_trace = X_svc + X_AA 와 PoK 를 확인한다 — proveShare/verifyShare.
...
/** 조각 비밀 x 의 공개점 X = x·B8. CIA 가 저장된 x_AA 로 승인 응답의 X_AA 를 다시 만들 때 쓴다. */
export async function sharePublicKey(x) {
  checkScalar(x, 'x');
  const bj = await getBj();
  return toObj(bj, bj.mulPointEscalar(bj.Base8, x));
}

// ---- CIA 조각의 Schnorr 지식 증명 — rogue key 방지(2026-09-16 스펙 §2 갱신, 2026-09-21) ----
// PoK{ x : X = x·B8 }, Fiat–Shamir. 챌린지는 (arid, X_svc) 를 덮어 이 등록에 묶인다 — 다른 서비스의 증명을 재사용할 수 없다.
// 모델 밖(honest-but-curious CIA)의 공격을 막는 값싼 보강이다: CIA 가 X_svc 를 보고 X_AA = X′ − X_svc 로 고르면
// x_AA 를 모르므로 이 증명을 낼 수 없다. 서비스는 pk_trace = X_svc + X_AA 까지 같이 확인해야 한다(verifyShare 가 하지 않는다).
export const DOMAIN_MODE3_SHAREPOK = 6125100363120046434005805977419n;   // ASCII "MODE3SHAREPOK" 빅엔디언
const MASK_250 = SCALAR_MAX - 1n;

function ctxFields(ctx) {
  if (typeof ctx?.arid !== 'bigint' || ctx.arid < 0n || ctx.arid >= SCALAR_MAX) throw new Error('share PoK: ctx.arid 는 [0, 2^250) bigint');
  if (typeof ctx?.X_svc?.x !== 'bigint' || typeof ctx?.X_svc?.y !== 'bigint') throw new Error('share PoK: ctx.X_svc{x,y}(bigint) 가 필요하다');
  return [ctx.arid, ctx.X_svc.x, ctx.X_svc.y];
}
async function shareChallenge(X, T, ctx) {
  const ps = await getPs();
  return ps.F.toObject(ps([DOMAIN_MODE3_SHAREPOK, ...ctxFields(ctx), X.x, X.y, T.x, T.y])) & MASK_250;
}
/** Z_r 균일 — 512비트를 뽑아 mod r(편향 2^-260). lib/mode3_issuance.js 의 randomZr 와 같은 이유. */
async function randomZr(bj) {
  return BigInt('0x' + randomBytes(64).toString('hex')) % bj.subOrder;
}

/** CIA 가 자기 조각 x 로 낸다. 결과 { X, T, c, z } — X = x·B8 도 같이 돌려준다. */
export async function proveShare(x, ctx) {
  checkScalar(x, 'x');
  const bj = await getBj();
  const X = toObj(bj, bj.mulPointEscalar(bj.Base8, x));
  const k = await randomZr(bj);
  const T = toObj(bj, bj.mulPointEscalar(bj.Base8, k));
  const c = await shareChallenge(X, T, ctx);
  const z = (k + c * x) % bj.subOrder;
  return { X, T, c, z };
}

/** 서비스가 확인한다. X 는 항등원이 아닌 부분군 점, T 는 부분군 점, z·B8 == T + c·X. 형식이 깨져도 false. */
export async function verifyShare(X, pok, ctx) {
  try {
    const bj = await getBj();
    if (!(await isTracePoint(X))) return false;
    const { T, c, z } = pok ?? {};
    if (typeof T?.x !== 'bigint' || typeof T?.y !== 'bigint' || T.x < 0n || T.x >= bj.F.p || T.y < 0n || T.y >= bj.F.p) return false;
    const Tp = fromObj(bj, T);
    if (!bj.inCurve(Tp) || !bj.inSubgroup(Tp)) return false;
    if (typeof c !== 'bigint' || c < 0n || c >= SCALAR_MAX) return false;
    if (typeof z !== 'bigint' || z < 0n || z >= bj.subOrder) return false;
    if ((await shareChallenge(X, T, ctx)) !== c) return false;
    const lhs = bj.mulPointEscalar(bj.Base8, z);
    const rhs = bj.addPoint(Tp, bj.mulPointEscalar(fromObj(bj, X), c));
    return bj.F.eq(lhs[0], rhs[0]) && bj.F.eq(lhs[1], rhs[1]);
  } catch { return false; }
}
