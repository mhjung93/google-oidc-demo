...
import { PEDERSEN_GENERATORS, SCALAR_MAX } from './mode3_credential.js';
...
/**
 * 발급 요청의 사용자 서명 메시지 = Poseidon(C_pt.x, C_pt.y, height) (설계 §6.2 단계 2).
 * 지갑의 signUserRequest 와 CIA 의 검증이 이 함수 하나를 공유한다 — 회로에는 들어가지 않는다.
 */
export async function issueRequestMessage(C_pt, height) {
  const ps = await getPs();
  return ps.F.toObject(ps([C_pt.x, C_pt.y, height]));
}
