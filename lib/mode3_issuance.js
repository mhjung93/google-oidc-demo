...
export const DOMAIN_MODE3_ISSUEREQ_V2 = 401414577397388343646241740924474930n;   // ASCII "MODE3ISSUEREQV2" (2026-09-18 전반, 폐기)
// 2026-09-18 후반: 지갑이 max_height 를 정하므로 요청 서명이 그 값도 덮는다(설계 §3.3 갱신). 재생으로 max_height 를 바꿔치기하지 못하게.
export const DOMAIN_MODE3_ISSUEREQ_V3 = 401414577397388343646241740924474931n;   // ASCII "MODE3ISSUEREQV3"
...
/**
 * 발급 요청의 사용자 서명 메시지 = Poseidon(DOMAIN_MODE3_ISSUEREQ_V3, C_pt.x, C_pt.y, chainid, allowAgent, max_height)
 * (설계 2026-09-18 §3.3). max_height 는 지갑이 정한 값이다(§3.2). r_s 는 빠졌다 — 재생으로 얻는 자격증명은 같은 C_pt 에 묶여 sk_i 없이는 쓸 수 없다.
 * 지갑의 signUserRequest 와 CIA 의 검증이 이 함수 하나를 공유한다 — 회로에는 들어가지 않는다.
 */
export async function issueRequestMessage(C_pt, chainid, allowAgent, max_height) {
  for (const [k, v] of [['chainid', chainid], ['allowAgent', allowAgent], ['max_height', max_height]]) {
  ...
  if (max_height >= MAX_HEIGHT_MAX) throw new Error('issueRequestMessage: max_height 는 2^64 미만이어야 한다');
  const ps = await getPs();
  return ps.F.toObject(ps([DOMAIN_MODE3_ISSUEREQ_V3, C_pt.x, C_pt.y, chainid, allowAgent, max_height]));
}
