...
/**
 * Sign(sk_u, (C_pt, height)): 등록된 장기키로 서명 (§6.2 단계 2). 메시지는 issueRequestMessage().
...
export async function signUserRequest(sk_uHex, C_pt, height) {
  ...
  const F = eddsa.F;
  const s = eddsa.signPoseidon(Buffer.from(sk_uHex, 'hex'), F.e(await issueRequestMessage(C_pt, height)));
