...
// 자격증명은 로그인마다 발급된다(2026-09-15 §5). 자격증명 V4 는 (C, max_height, chainid, allowAgent) 위 서명이다(2026-09-18 §3).
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { ethers } from 'ethers';
import { buildEddsa } from 'circomlibjs';
import * as snarkjs from 'snarkjs';
import { randomScalar, ppid, normalizeAttrs, MAX_HEIGHT_MAX } from './mode3_credential.js';
...
/** Sign(sk_u, (C_pt, chainid, allowAgent, max_height)): 등록된 장기키로 서명 (설계 2026-09-18 §3.3). 메시지는 issueRequestMessage(). */
export async function signUserRequest(sk_uHex, C_pt, chainid, allowAgent, max_height) {
  if (typeof chainid !== 'bigint' || typeof allowAgent !== 'bigint' || typeof max_height !== 'bigint') throw new Error('signUserRequest: chainid·allowAgent·max_height(bigint) 가 필요하다');
  const eddsa = await getEddsa();
  const F = eddsa.F;
  const s = eddsa.signPoseidon(Buffer.from(sk_uHex, 'hex'), F.e(await issueRequestMessage(C_pt, chainid, allowAgent, max_height)));
  return { R8x: F.toObject(s.R8[0]).toString(), R8y: F.toObject(s.R8[1]).toString(), S: s.S.toString() };
}

/** §5 단계 1~2. blind 는 여기서 새로 뽑고 secrets 로 돌려준다. allowAgent 는 사용자의 선택(2026-09-18 §3.1), 기본 0. max_height 는 지갑이 정한다(chooseMaxHeight). */
export async function buildIssueRequest({ uid, arid, s_u, r_u, sk_u, session, chainid, attrs, allowAgent = 0n, max_height }) {
  ...
  const body = {
    ...
    sig_u: await signUserRequest(sk_u, C_pt, chainid, allowAgent, max_height), chainid: chainid.toString(), allowAgent: allowAgent.toString(),
...
export async function buildCredentialProof({ uid, arid, s_u, blind, pk_i, attrs, credential, pk_CIA, pk_trace, tree, wasmPath = WASM_PATH, zkeyPath = ZKEY_PATH }) {
  ...
  const tag = await encryptTag(pk_trace, uid, arid);
  const input = {
  ...
  return { proof, publicSignals, revRoot: tree.getRoot(), tag: { c1: tag.c1, c2: tag.c2, h: tag.h } };
}
