 ...
 * keccak256(abi.encode(chainid, wallet, to, value, data, nonce, discMask, discLo, discHi)).
 * discMask/discLo/discHi 는 π 의 pub[14]·pub[15..18]·pub[19..22](2026-09-22 §5.1) — 다이제스트가 공개 값
 * 9워드 전부를 덮어야 같은 세션키·같은 mask 의 π 를 다른 대상용 lo/hi 로 바꿔 끼우지 못한다(리뷰 수정 1).
 */
export function payloadDigest({ chainId, wallet, to, value, data, nonce, discMask = 0n, discLo = [0n, 0n, 0n, 0n], discHi = [0n, 0n, 0n, 0n] }) {
  return ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
    ['uint256', 'address', 'address', 'uint256', 'bytes', 'uint256', 'uint256', 'uint256[4]', 'uint256[4]'],
    [chainId, wallet, to, value, data, nonce, discMask, discLo, discHi],
  ));
}
...
/** snarkjs 증명을 execute 인자(a, b, c, pub) 로. pub 은 hex 문자열 23개 — ethers 가 uint256 으로 받는다. */
...
export function parseExecuteReceipt(receipt, walletAddress = null) {
  ...
  for (const l of receipt.logs ?? []) {
    ...
    if (p.name === 'Disclosure') disclosure = { mask: p.args.mask, lo: [...p.args.lo], hi: [...p.args.hi] };
  }
...
/** 데모 대상(§5.3)을 배포한다. factoryAddress 로 isWallet 을 확인해 팩토리가 배포한 지갑의 꼬리만 받는다. */
