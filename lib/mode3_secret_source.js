...
import { registrationCommit } from './mode3_issuance.js';
...
/**
 * 증인 형식 검사. `publicCm` (파일의 공개 `registration.cm_u`)을 주면 **증인이 그 등록에 실제로 묶였는지**까지 본다 —
 * cm_u = s_u·G₃ + r_u·H 가 등록 때 CIA 에 낸 점과 같아야 한다(2026-09-22 최종 리뷰 Minor 1). 이 검사가 없으면
 * 형식만 맞는 아무 s_u·r_u 나 받아 이 지갑의 등록과 무관한 증명 시도로 이어진다. cm_u 계산이 비동기라 이 함수도 비동기다.
 */
export async function validateWitness(w, publicUid, publicCm = null) {
  ...
  if (publicCm) {
    if (!isPt(publicCm)) fail('등록의 cm_u 형식');
    const cm = await registrationCommit(BigInt(w.s_u), BigInt(w.r_u));
    if (cm.x.toString() !== publicCm.x || cm.y.toString() !== publicCm.y) fail('cm_u 가 등록과 다르다(s_u·r_u 불일치)');
  }
}
