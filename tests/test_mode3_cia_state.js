...
t('v5 → v6: issued 를 버리고 각 계정에 creds:[] 를 넣는다', () => {
...
t('v4 → v6: issued 를 버리고(V3 서명은 죽은 자격증명) openings 에 resolved·allowAgent·max_height·chainid 를 채운다, 계정에 creds:[], 나머지는 그대로', () => {
  const v4 = {
    version: 4,
  ...
  assert.deepEqual(state.accounts, { '12345': { ...v4.accounts['12345'], creds: [] } }); assert.deepEqual(state.rps, v4.rps);
...
t('v3 → v6: used_rs 버림, rps 는 approved·조각 없음, openings [] (v4·v5 규칙을 거쳐 온다)', () => {
  ...
  assert.equal(state.version, 6); assert.equal(state.used_rs, undefined); assert.equal(state.issued, undefined);
...
t('v6 는 그대로(notes 없음), v2 이하·미래 버전은 throw', () => {
  const { state, notes } = migrateCiaState(defaultCiaState());
  assert.equal(state.version, 6); assert.deepEqual(notes, []);
  assert.throws(() => migrateCiaState({ version: 2 }), /version 2/);
  assert.throws(() => migrateCiaState({ version: 7 }), /version 7/);
});
