...
t('기본 상태는 v8 이고 issued 가 없다', () => {
  const s = defaultCiaState();
  assert.equal(s.version, 8); assert.equal(CIA_STATE_VERSION, 8);
  assert.deepEqual(s, { version: 8, accounts: {}, rps: {}, openings: [], revoked: [], pending: [], epoch: 0 });
});

t('v5 → v8(v6·v7 을 거쳐): issued 를 버리고 각 계정에 creds:[]·sessions:[] 를 넣는다', () => {
  ...
  assert.equal(state.version, 8); assert.equal(state.issued, undefined);
...
t('v4 → v8(v6·v7 을 거쳐): issued 를 버리고(V3 서명은 죽은 자격증명) openings 에 resolved·allowAgent·max_height·chainid 를 채운다, 계정에 creds:[]·sessions:[], 나머지는 그대로', () => {
  const v4 = {
    version: 4,
  ...
  assert.deepEqual(state.accounts, { '12345': { ...v4.accounts['12345'], creds: [], sessions: [], attrs: ['0', '0', '0', '0'] } }); assert.deepEqual(state.rps, v4.rps);
...
t('v3 → v8(v6·v7 을 거쳐): used_rs 버림, rps 는 approved·조각 없음, openings [] (v4·v5 규칙을 거쳐 온다)', () => {
  ...
  assert.equal(state.version, 8); assert.equal(state.used_rs, undefined); assert.equal(state.issued, undefined);
...
t('v8 는 그대로(notes 없음), v2 이하·미래 버전은 throw', () => {
  const { state, notes } = migrateCiaState(defaultCiaState());
  assert.equal(state.version, 8); assert.deepEqual(notes, []);
  assert.throws(() => migrateCiaState({ version: 2 }), /version 2/);
  assert.throws(() => migrateCiaState({ version: 9 }), /version 9/);
});

t('v6 → v8(v7 을 거쳐): attrs 가 채워지고 활성 자격증명은 전부 물려 revoked·pending 에 리프가 들어간다', () => {
...
t('v8 기본 상태와 demoAttrs 없는 계정은 0 네 개', () => {
  assert.equal(defaultCiaState().version, 8);
