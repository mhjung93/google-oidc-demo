...
async function getCode(codeChallenge, redirectUri) {
  ...
  // 동의는 로그인한 브라우저 세션에 묶인다(2026-09-04) — 세션 쿠키를 들고 다녀야 한다.
  ...
  const consent = await (await fetch(`${IDP}/authorize/consent`, {
    method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: idpSession },
...
async function testReplayRejected() {
  ...
  async function runOnce(suffix) {
    ...
    // 동의는 로그인한 브라우저 세션에 묶인다(2026-09-04).
    const loginRes2 = await fetch(`${IDP}/authorize/login`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ request_uri: par.request_uri, username: 'testuser', password: 'password123' }),
    });
    const idpSession2 = (loginRes2.headers.get('set-cookie') || '').split(';')[0];
    const consent = await (await fetch(`${IDP}/authorize/consent`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: idpSession2 },
      body: JSON.stringify({ request_uri: par.request_uri, allowed: true }),
    })).json();
