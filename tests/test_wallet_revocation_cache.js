...
// fetchRevocationWitnessesV2()가 캐시 판정보다 앞에 있으면, 증명을 재사용하는
...
const fetchWitnessCallIndex = walletAgentSourceForOrderCheck.indexOf(
  'rev = await fetchRevocationWitnessesV2(',
);

assert.notEqual(cacheHitCheckIndex, -1, 'cache-hit check not found in wallet_agent.js');
assert.notEqual(fetchWitnessCallIndex, -1, 'fetchRevocationWitnessesV2() call site not found in wallet_agent.js');
assert(
  cacheHitCheckIndex < fetchWitnessCallIndex,
  'the cached-proof check must come BEFORE fetchRevocationWitnessesV2() — otherwise every ' +
