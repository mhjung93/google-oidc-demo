...
add_bullet_slide("③ 컨트랙트가 트리를 유지하게 하면?", [
    ...
    (1, "삽입마다 root 재계산: Poseidon 약 40회. EVM에 precompile이 없어 실측 기준 폐기 1건당 약 216만 gas다", True),
...
add_bullet_slide("남은 위험과 전제", [
    ...
    (1, "추정 항목이 없다. 온체인 Poseidon도 실측했다 — 해시 1회 54,138 gas로 keccak(4,771)의 11.3배", True),
    (0, "가정에 의존한다", True),
...
add_bullet_slide("진행 상황과 남은 것", [
    ...
    (1, "온체인 Poseidon도 실측 완료 — keccak의 11.3배라 상위 트리를 밖으로 뺀 판단이 수치로 뒷받침된다"),
])
