...
add_title_slide(
    ...
    "PairCT Mode 2 폐기 구조 설계 검토 · 2026-09-04 (2026-09-05 개정)",
)
...
add_table_slide(
    ...
    ["", "담는 것", "언제 죽나", "비용"],
    [
        ["SNARK", "target ∉ 내 서브트리\n(세션·계정 각 1건)", "내 서브트리가 바뀔 때만", "약 546 ms"],
...
add_table_slide(
    ...
    [
        ...
        ["증명 시간", "755.8 ms", "약 546 ms", "환산 (미실측)"],
        ["execute 가스", "276,709", "+약 12,000 (keccak 22회 + calldata)", "추정"],
...
add_bullet_slide("남은 위험과 전제", [
    ...
    (1, "증명 시간 546ms는 constraint 비로 환산한 값이다. 실측하려면 새 zkey와 샤드를 아는 witness 생성기가 필요하다", True),
    (1, "가스 +12,000과 온체인 Poseidon 100만 gas는 추정이다 — 후자는 poseidon_gencontract를 배포해 재봐야 한다"),
    (0, "가정에 의존한다", True),
...
# 18. 다음 단계
# ============================================================
add_bullet_slide("다음 단계", [
    (0, "1. constraint 실측 — 완료 (2026-09-04~05)", True),
    (1, "3,732 / 4,218 / 13,905 확정. results/mode2_sharded_circuit_constraints_20260904.csv"),
    (0, "2. 설계 문서 작성 (docs/superpowers/specs/)", True),
    (1, "2축 세션 샤딩, 블록 단위 grace, 소트니스 조건 4개, 샤드별 epoch 동기화 프로토콜, 갱신 정당성 증명"),
    (0, "3. 현재 설계를 논문에 먼저 반영", True),
    (1, "초안(final46)에 폐기 절·Property·Table 1 열이 통째로 없다", True),
    (1, "이 샤딩 설계는 그 다음이다 — 지금 것부터 써야 비교 기준이 생긴다"),
    (0, "4. 남은 실측", True),
    (1, "새 zkey + 샤드를 아는 witness 생성기로 증명 시간, poseidon_gencontract로 온체인 해시 가스"),
    (0, "5. 구현", True),
    (1, "lib/ 신규 모듈 + 단위 테스트 → IdP/지갑 배선 → zkey·재배포"),
])
