...
def add_membership_contrast_slide():
    ...
    _caption(
        ...
        [
            ...
            (0, "대신 root가 바뀌는 빈도가 다르다: BAAR는 발급+폐기, 우리는 폐기만. 23번에서 이 손익분기를 본다."),
        ],
    )
    return slide
...
add_bullet_slide("오늘의 순서와, 근거를 표시하는 방식", [
    ...
    (1, "⑥ 평가와 수치, 그리고 그 안의 모순 (17~20)"),
    (1, "⑦ 비판적 검토와 우리 설계와의 대조 (21~27)"),
    (0, "근거 표시 규칙 — 섞이면 논의가 흐려진다", True),
    (1, "\"원문\": 논문 본문에 있는 내용 — documents/BAAR_PLOSONE_e0343696.pdf로 직접 대조했다"),
    ...
    (1, "13~14번(핵심 선택)까지를 25분 지점 통과 목표로, 21~27번(비판·대조)에 최소 25분을 남긴다"),
])
...
add_bullet_slide("기존 접근의 한계 — 논문이 지목하는 것", [
    ...
    (1, "그래서 SNARK가 아니라 Schnorr Σ-protocol을 고른다 — 24번에서 이 선택의 대가를 본다"),
])
...
add_table_slide(
    ...
    note=[
        (0, "발표자 분석: 검증자가 오프체인 주체라는 점을 기억해 두자. 우리 시스템은 검증자가 스마트컨트랙트다 — 24번에서 이 차이가 결정적으로 작동한다."),
...
add_bullet_slide("구성요소 ①② Pedersen 커밋먼트와 Schnorr ZKPoK (원문)", [
    ...
    (1, "증명 비용은 생성원 개수, 즉 속성 수 n에 선형이다 — 20번의 sublinear 비판과 이어진다", True),
])
...
add_bullet_slide("구성요소 ③ 머클 동적 누산기 (원문)", [
    ...
    (1, "개별 커밋먼트는 체인에 저장하지 않는다 — 가스가 낮은 이유이자, 22번 비판의 뿌리다", True),
    (0, "발표자 분석", True),
...
add_bullet_slide("프로토콜 ④ 폐기 — 리프를 뺀다 (원문)", [
    ...
    (0, "원문은 폐기를 세 연산으로 나눈다 — 우리 계정 층과 대응된다", True),
    (1, "Deactivate(식별자를 RCL에 추가) · Revoke(RCL 갱신) · Recover(재발급). 우리의 disabled / 크레덴셜 폐기 / unpin과 같은 삼분이다"),
    (1, "Table 3 실측 가스: Revocation 31,238 · Deactivation 50,753 · Recovery 28,810 · Verify 28,083"),
    (0, "복잡도와 지연 (원문)", True),
    (1, "Table 4: Revocation = O(d)H (d = 트리 깊이)"),
    (1, "시간은 세 값이 따로 나온다(Table 3 0.0182 s, Fig 3 ≈180 ms, Fig 6 200→310 ms) — 20번에서 따진다"),
    (0, "보안 근거 (원문 Lemma 5)", True),
    (1, "폐기된 크레덴셜에 대해 유효한 witness는 존재하지 않는다 — 만들려면 해시 충돌이 필요하다"),
    (0, "발표자 분석: 제거 즉시 효력이 생긴다. 유예 창이 없다 — 이 점은 우리보다 낫다(26번)", True),
])
...
add_table_slide(
    ...
    note=[
        (0, "원문 Protocol 3: \"Leaves form a Merkle tree with a root Acc, representing all valid credentials. To revoke, the CIA removes x and publishes Acc′ = MerkleRoot(X ∖ {x})\", 인증은 MerkleVerify(Acc′, x, π) = 1 — 유효 집합 멤버십이다."),
        (0, "그런데 같은 논문의 알고리즘 명세는 다른 자료구조를 쓴다: RCL = \"on-chain Revoked Credential List\"(초기값 ∅), Deactivate/Revoke는 \"adding its identifier to the revocation list\". 이름도 방향도 반대인 둘이 공존한다."),
        (0, "발표자 판단: 인증식이 Acc를 쓰므로 실제 프로토콜은 멤버십이다. RCL은 표기만 남고 프로토콜·증명에 쓰이지 않는다 — 우리 내부 문서(§3.1 표·§3.3 트리거)가 이 RCL 쪽 서술을 받아 적어 반대로 정리돼 있다."),
    ],
)
...
add_table_slide(
    "성능 ① 논문의 온체인 실측 (원문 Table 3)",
    ["연산", "Gas", "Transaction time (s)"],
    [
        ["Setup", "N/A", "0.0001"],
    ...
    [Inches(4.0), Inches(4.0), Inches(4.3)],
    height=Inches(3.4), font_size=13,
    note=[
        (0, "이것이 논문 자신의 측정표다. 뒤의 비교 그림(Fig 3·4)과 값이 어긋나는 대목이 있어 20번에서 따진다."),
    ],
)

add_bullet_slide("성능 ② 비교 그림이 말하는 것 (원문 Fig 3·4·5·6·8)", [
    (0, "시간 (Fig 3, 비교군 [36]·[37])", True),
    (1, "Setup 120 / 190 / 160 ms · KeyGen 150 / 23 / 210 ms · Revocation 180 / 280 / 250 ms · Verification 200 / 350 / 320s"),
    (0, "가스 (Fig 4, 비교군 [37])", True),
    (1, "Create ≈110K vs ≈135K · Verify ≈60K vs ≈100K · Revocation ≈30K vs ≈270K. \"가스 50~90% 절감\" 주장"),
    (0, "폐기 확장성 (Fig 5·6, 비교군 [14]) — 논문은 \"two mutually valid dimensions\"라 부른다", True),
    (1, "Fig 5 누적: [14]는 5,000건에서 약 5초로 선형, BAAR는 \"remains constant nonetheless of the number\""),
    (1, "Fig 6 건당: [14]는 800–950 ms에서 안정화, BAAR는 200 ms → 310 ms(5,000건)"),
    (0, "속성 확장성 (Fig 7·8, 비교군 [15])", True),
    (1, "Fig 7 validation: BAAR 0.018 s vs [15] 0.028 s. Fig 8: 속성 8~1024에서 [15]는 15초 → 33초, BAAR는 \"sublinear and stable\""),
])

add_bullet_slide("성능 ③ 그 수치들이 서로 맞지 않는다 (발표자 분석)", [
    (0, "① 같은 연산에 값이 둘씩 있다", True),
    (1, "Verify 가스: Table 3은 28,083인데 Fig 4는 ≈60K — 2배 차이다"),
    (1, "Validation 시간: Table 3은 0.0669 s인데 Fig 7은 0.018 s"),
    (1, "Revocation 시간: Table 3 0.0182 s · Fig 3 ≈180 ms · Fig 6 200~310 ms — 셋 다 다르고 정의가 없다"),
    (0, "② Fig 5와 Fig 6이 동시에 참일 수 없다", True),
    (1, "건당 지연이 200 → 310 ms로 오르는데 누적이 \"일정\"할 수는 없다. 누적은 건당의 합이다"),
    (0, "③ 우월성 서술이 자기 수치와 어긋난다", True),
    (1, "KeyGen에서 [36]은 23 ms인데 BAAR는 150 ms다. 논문은 그런데도 \"BAAR also exhibits superior efficiency\"라고 적는다(6.5배 느리다)"),
    (1, "Verification 비교의 \"320s\"는 단위 오류로 보인다(320 ms). Fig 7 서술의 \"36 percent higher\"도 실제로는 낮은 값이다"),
    (0, "④ sublinear 주장이 자기 복잡도 표와 충돌한다", True),
    (1, "Table 4: Authentication = ((m+3)SM + 2PA + (d+2)H + S) — 속성 수 m에 선형이다(SM = scalar multiplication)"),
    (1, "그리고 [15]의 15→33초는 논문 스스로 \"inherent SNARK generation\"에 귀속한다 — 증명 생성 시간을 BAAR의 검증 시간과 나란히 놓은 것이다"),
])
...
add_bullet_slide("비판 ② 손익분기 — 갱신 트래픽 대 회로 비용", [
    ...
    (1, "단 constraints 69%가 시간 69%는 아니다 — 4,820 → 19,251(4.0배)일 때 295.5 → 755.8 ms(2.56배)다. 20번에서 BAAR를 때린 논리를 우리도 지켜야 한다", True),
...
add_bullet_slide("정리와 논의거리", [
    ...
    (1, "witness 갱신의 전달 채널이 설계·평가 어디에도 없다 — CIA 상시 의존이 숨어 있다(22번)"),
    (1, "unlinkability 주장과 프로토콜이 어긋난다 — C와 witness를 평문으로 넘기므로 검증자 하나로 세션 연결이 된다(16번)", True),
